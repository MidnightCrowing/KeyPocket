...
public static class ExchangeRateHelper
{
    ...
    public static decimal? Convert(decimal amount, string fromCurrency, string toCurrency)
    {
        ...
        var directKey = $"{fromCurrency.ToUpper()}_{toCurrency.ToUpper()}";
        if (rates.TryGetValue(directKey, out var rate)) return amount * rate;

        // 3. 查找反向汇率 (Target -> Source)，取倒数
        var reverseKey = $"{toCurrency.ToUpper()}_{fromCurrency.ToUpper()}";
        if (rates.TryGetValue(reverseKey, out var reverseRate) && reverseRate != 0) return amount / reverseRate;
    ...
    public static string GetCurrencySymbol(string currencyCode)
    {
        ...
        if (SettingsHelper.Current.CurrencySymbols.TryGetValue(code, out var symbol)) return symbol;
        return code + " ";
    }
}